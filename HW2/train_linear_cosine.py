...
if __name__ == '__main__':
    ...
    print("Linear Search using cosine distance with resubstitution method training time is %s seconds" % used_time)
    start_time = time.time()
    predicted_rs = KNN_rs.predict(X)
    used_time = time.time() - start_time
    print("Linear Search using cosine distance with resubstitution method querying time is %s seconds" % used_time)
    cfm_rs = confusion_matrix(Y, predicted_rs, labels=range(1, 11))
    print("Linear Search using cosine distance with resubstitution method confusion matrix is ")
    print(cfm_rs)
    ac_rate = accuracy_score(Y, predicted_rs)
    print("Linear Search using cosine distance with resubstitution method accurary score is %s" % ac_rate)
    plt.figure()
    plot_confusion_matrix(cfm_rs, classes=range(1, 11),
                          title='Linear Search using cosine distance with resubstitution method')
    ...
    plot_confusion_matrix(cfm_kfold, classes=range(1, 11),
                          title='Linear Search using cosine distance with KFold method')
    plt.show()

...
if __name__ == '__main__':
    ...
    KNN_rs = KNeighborsClassifier(n_neighbors=5, algorithm='brute', metric='manhattan')
    start_time = time.time()
    KNN_rs.fit(X, Y)
    used_time = time.time() - start_time
    print("Linear Search using manhattan distance with resubstitution method training time is %s seconds" % used_time)
    start_time = time.time()
    predicted_rs = KNN_rs.predict(X)
    used_time = time.time() - start_time
    print("Linear Search using manhattan distance with resubstitution method querying time is %s seconds" % used_time)
    cfm_rs = confusion_matrix(Y, predicted_rs, labels=range(1, 11))
    print("Linear Search using manhattan distance with resubstitution method confusion matrix is ")
    print(cfm_rs)
    ac_rate = accuracy_score(Y, predicted_rs)
    print("Linear Search using manhattan distance with resubstitution method accurary score is %s" % ac_rate)
    plt.figure()
    plot_confusion_matrix(cfm_rs, classes=range(1, 11),
                          title='Linear Search using manhattan distance with resubstitution method')
    ...
    for train_index, test_index in kf.split(X):
        ...
        clf_KFold = KNeighborsClassifier(n_neighbors=5, algorithm='brute', metric='manhattan')
        start_time = time.time()
    ...
    plot_confusion_matrix(cfm_kfold, classes=range(1, 11),
                          title='Linear Search using manhattan distance with KFold method')
    plt.show()
